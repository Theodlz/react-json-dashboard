...
const JsonDashboard = ({ title, data, statusTypes, showTitle, lastUpdated, refreshButton }) => {
  if ((!data || typeof data !== "object" || Object.keys(data).length === 0) && !refreshButton) {
    return null;
  }
  return (
    <div style={{ width: "100%", height: "100%" }}>
        <div style={{ display: "flex", justifyContent: "space-between", flexDirection: "row", alignItems: "center", height: "2.2rem" }}>
            {showTitle ? (
            <h1 style={{ margin: 0, padding: 0, fontSize: "1.8rem" }}>{title}</h1>
            ) : (<></>)}
            {refreshButton && (
            refreshButton
            )}
        ...
        {lastUpdated && (
        <h1 style={{fontSize: "1rem", fontStyle: "italic", margin: 0, padding: 0, marginTop: "-0.3rem", marginBottom: "0.3rem"}}>
            Last updated: {typeof lastUpdated === "string" ? (lastUpdated.split('.')[0] || lastUpdated) : lastUpdated.toISOString().split('.')[0]}
        </h1>
        )}
      <div style={{ display: "flex", flexDirection: "column" }}>
        {typeof data === 'object' && Object.keys(data).length > 0 && Object.keys(data).map((category) => {
          if (
            data[category] !== null &&
